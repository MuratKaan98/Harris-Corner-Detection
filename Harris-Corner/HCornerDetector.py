...
def gaussian_mask(n, sigma=None):   #apply gaussian blur to smoothen the image.
    if sigma is None:
        sigma = 0.3 * (n // 2) + 0.8
...
def seperable_conv(I, filter_x, filter_y):   #Image derivatives, Sobel filter definition
    h, w = I.shape[:2]
...
def detect(I, n_g=5, n_w=5, k=0.06):    #for Sobel derivatives, we just take the filters as ([-1, 0, 1]) and ([1, 2, 1])
    ...
    D_temp = np.zeros((h, w, 2, 2))     #Using the smoothened derivatives $I_x$ and $I_y$ we calculate the structure tensor A at every pixel.
    ...
    P = D[:, :, 0, 0]   #Eigenvalues calculation
    Q = D[:, :, 0, 1]
    R = D[:, :, 1, 1]
    T1 = (P + R) / 2
    ...
    C = L_1 * L_2 - k * np.square(L_1 + L_2)   #Cornerness measure C was calculated
